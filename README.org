...
2. Mutating `df` inside the function doesn't affect the global `gapminder` data frame. What happens in the function stays in the function; this is called scope.
...
Briefly describe split-apply-combine
aggregate files to a single dataframe
