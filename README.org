...
* TODO Move subsetting back to, or near, its original location
* TODO Clean up ordering and/or reduce redundancy of matrix and data frame sections throughout lesson
...
6. ~apply~ lets you apply an arbitrary function to an abitrary subset of a matrix. This is an example of a higher-order function (map, apply, filter, reduce, fold, etc.)
   #+BEGIN_SRC R
   apply(m, 1, mean)
   apply(m, 2, mean)
   apply(m, 1, sum)
