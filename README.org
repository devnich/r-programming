...
1. ~apply()~ apply function over margins of array
2. ~lapply()~ apply function over list, returning list
3. ~sapply()~ apply function polymorphically over list, returning vector, matrix, or array

4. ~apply~ lets you apply an arbitrary function to an abitrary subset of a matrix. This is an example of a higher-order function (map, apply, filter, reduce, fold, etc.) that can (and should) replace loops for most purposes
   #+begin_src R
   apply(m, 1, mean)
   apply(m, 2, mean)
   apply(m, 1, sum)
...
5. The ~~.x~ operation in Purrr creates an anonymous function that applies to all the elements in the collection (in general, the Tidyverse uses ~.~ as a shorthand for "for each element")
