...
1. The ~~.x~ operation in Purrr creates an anonymous function that applies to all the elements in the collection (in general, the Tidyverse uses ~.~ as a shorthand for "for each element")
...
* *WEEK 3: Tidyverse*
* Pipes
** COMMENT Read files using apply with pipes
NB: this is an impoverished example
